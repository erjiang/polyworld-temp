...
void genome::genomeinit()
{
    ...
    // neurons (thus applying to the output/behavior neurons only), 
	// they are considered exclusively excitatory.  This is a bit of a hack
    // to permit input and output neurons to be used as inhibitors, without
    // going through a separate, signal-inverting group, primarily
    // for the sake of computational efficiency.

    // Correspondingly, since the second batch of neuronal groups (7 originally)
    // are designated as the "output" (behavior) groups, the number of neurons
    // (again doubling as both excitatory and inhibitory) in these groups is
    // fixed (1); hence only maxneurgroups-numoutneurgroups-numinputneurgroups
	// specifications are needed to determine all the non-input group sizes.
	// On the other hand, the output group's neurons do accept inputs as well as
	// being allowed to feedback their activation levels to other non-input groups,
	// hence their inclusion as just another non-input group (rather than
    // separating them out as is done with the input groups).
	...
	// The ClusterNeuralGroupGenes flag controls whether we use the new gene layout,
	// in which all the genes associated with a particular neural group are clustered
	// together, or the old gene layout, in which genes are clustered based on what
	// they specify (eecd genes all together, eicd genes all together, etc.).

#if ClusterNeuralGroupGenes

	// location of the excitatory neuron count (within each neural group's block of genes)
	genome::g
...
void genome::PrintGeneIndexes( FILE* f )
{
	...
	fprintf( f, "%ld\tCrossoverPointCount\n", ncptsgene );
